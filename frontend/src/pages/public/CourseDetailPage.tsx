...
export const CourseDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const enrolled = enrollmentsData.myEnrollments.some(
      (enroll: any) => enroll.course.courseId === courseId
    );
  ...
  const lessons = useMemo(() => {
    return modules.flatMap((module: any) => module.lessons?.map((lesson: any) => ({
      ...lesson,
        ...
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <div className="flex flex-wrap gap-3 text-sm font-medium items-center">
              ...
              <span className="bg-gray-700 px-3 py-1 rounded">⏱ {formatMinutes(course.totalDuration)}</span>
        ...
        <div className="max-w-7xl mx-auto px-4 flex gap-4 overflow-x-auto">
          {tabs.map((tab) => (
            <button
              ...
              className={`py-4 px-3 text-sm font-semibold border-b-2 transition-colors ${selectedTab === tab.key ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-800'}`}
            >
              {tab.label}
            </button>
          ))}
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          {selectedTab === 'overview' && (
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              ...
              <p className="text-gray-700 leading-relaxed">{course.description || 'Khóa học giúp bạn nắm vững kiến thức cốt lõi và thực hành qua dự án thực tế.'}</p>
                    ...
                    <div className="divide-y divide-gray-100">
                      {module.lessons?.map((lesson: any) => (
                        <button
                          ...
                          className={`w-full text-left px-4 py-3 flex items-center gap-3 hover:bg-blue-50 transition-colors ${selectedLessonId === lesson.lessonId ? 'bg-blue-50 border-l-4 border-blue-600' : ''}`}
                        >
                          ...
                          <span className="text-xs text-gray-400">{formatMinutes(lesson.durationSeconds)}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                          ...
                          <div className="mt-3 pl-4 border-l-2 border-gray-300 space-y-2">
                            {comment.replies.map((reply: any) => (
                              <div key={reply.commentId} className="flex gap-3">
                                ...
                                />
                                <div>
                                  <div className="text-sm font-semibold text-gray-800">{reply.user?.fullName}</div>
                  ...
                  <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button
                        ...
                        className={`w-10 h-10 rounded-full border flex items-center justify-center ${reviewRating >= star ? 'bg-amber-100 border-amber-300 text-amber-600' : 'border-gray-200 text-gray-400'}`}
                      >
                        ⭐
                      </button>
                    ))}
              ...
              {selectedLesson && (
                <div className="space-y-4">
                  ...
                  {quizzesData?.getQuizzesByLesson?.map((quiz: any) => (
                    <div key={quiz.quizId} className="border border-gray-200 rounded-lg p-4 flex items-start justify-between gap-4">
                      <div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200 sticky top-24 space-y-4">
            ...
            <p className="text-gray-500 text-sm line-through">{(((course.price ?? 0) * 1.5)).toLocaleString()} đ</p>

...
import { 
  ...
  BarChart, DollarSign, Calendar, AlertCircle 
} from 'lucide-react';

// Import các định nghĩa GraphQL
...
export const MyCoursesPage = () => {
  ...
  // 1. Lấy danh sách khóa học thật từ Backend
  const { data, loading, error, refetch } = useQuery(GET_MY_COURSES_QUERY, {
    fetchPolicy: 'network-only', // Đảm bảo dữ liệu luôn mới nhất
  });

  // 2. Mutation Xóa khóa học
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Danh sách tất cả các khóa học bạn đã khởi tạo trên hệ thống.</p>
        </div>
        <button 
      ...
      {courses.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {courses.map((course: any) => (
            <div key={course.courseId} className="group bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-2xl hover:shadow-blue-50 transition-all duration-300 flex flex-col">
              ...
              <div className="h-48 bg-gray-100 relative overflow-hidden">
                ...
                <div className={`absolute top-4 right-4 px-3 py-1 rounded-full text-[10px] font-black tracking-widest shadow-sm ${
                  course.isPublished ? 'bg-green-500 text-white' : 'bg-gray-900/80 text-white backdrop-blur-md'
                }`}>
              ...
              <div className="p-6 flex-1 flex flex-col">
                <h3 className="font-bold text-gray-900 text-lg leading-tight mb-4 line-clamp-2 h-12" title={course.title}>
                  {course.title}
                </h3>

...
import { 
  ...
  TrendingUp, ArrowRight, Loader2, BookX, Edit3 
} from 'lucide-react';
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard 
            ...
            icon={Users} 
            color="bg-purple-500"
            subtext="Tính năng đang phát triển"
        />
        <StatCard 
                ...
                {recentCourses.length > 0 ? recentCourses.map((course: any) => (
                    ...
                    >
                        <div className="w-12 h-12 rounded-lg bg-gray-100 flex items-center justify-center text-gray-400 group-hover:bg-blue-50 group-hover:text-blue-500 transition-colors shrink-0">
                            {course.thumbnailUrl ? (
                                <img src={course.thumbnailUrl} className="w-full h-full object-cover rounded-lg" alt="" />
                            ) : (
                                <BookOpen size={20} />
                            )}
                        </div>
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-400 mt-1 flex items-center gap-2">
                                <span className={course.isPublished ? "text-green-600 font-medium" : "text-gray-500 font-medium"}>
                                    {course.isPublished ? 'Công khai' : 'Bản nháp'}
                                </span>
                                <span>• {course.totalLessons || 0} bài</span>
                            </p>
                        </div>
                        <Edit3 size={16} className="text-gray-300 group-hover:text-blue-500 shrink-0 mt-1" />
                    </div>
                )) : (

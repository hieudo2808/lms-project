...
import { 
  ...
  CREATE_LESSON_MUTATION, UPDATE_LESSON_MUTATION, DELETE_LESSON_MUTATION, REORDER_LESSONS_MUTATION 
...
const CurriculumEditor: React.FC<CurriculumEditorProps> = ({ courseId, modules, refetch }) => {
  ...
  useEffect(() => {
    ...
    const sortedModules = [...modules].sort((a, b) => a.order - b.order).map(m => ({
      ...m,
                      ...
                      <div className="flex items-center justify-between p-4 bg-gray-100 hover:bg-gray-200 group">
                        {editingModuleId === module.moduleId ? (
                          <div className="flex items-center gap-2 flex-1">
                            <input 
                              ...
                              autoFocus
                            />
                            <button onClick={() => handleUpdateModule(module.moduleId)} className="text-green-600"><Save size={18}/></button>
                            <button onClick={() => setEditingModuleId(null)} className="text-gray-500"><X size={18}/></button>
                          </div>
                        ) : (
                          <div className="flex items-center gap-3 flex-1 cursor-pointer" onClick={() => toggleModule(module.moduleId)}>
                            {/* Drag Handle cho Module */}
                            <div {...provided.dragHandleProps}>
                                <GripVertical size={20} className="text-gray-400 cursor-grab active:cursor-grabbing hover:text-gray-600" />
                            </div>
                            <button className="text-gray-500">
                              {expandedModules[module.moduleId] ? <ChevronDown size={20} /> : <ChevronRight size={20} />}
                            </button>
                        ...
                        {editingModuleId !== module.moduleId && (
                          <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => { setEditingModuleId(module.moduleId); setEditModuleTitle(module.title); }} className="text-blue-500 p-2 hover:bg-blue-50 rounded-full"><Edit2 size={16} /></button>
                            <button onClick={() => handleDeleteModule(module.moduleId)} className="text-red-500 p-2 hover:bg-red-50 rounded-full"><Trash2 size={16} /></button>
                          </div>
                        )}
                      </div>
                                                ...
                                                >
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div {...provided.dragHandleProps}>
                                                                <GripVertical size={16} className="text-gray-300 cursor-grab active:cursor-grabbing hover:text-gray-500" />
                                                            </div>
                                                            <div className={`p-2 rounded-full ${lesson.videoUrl ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'}`}>
                                                                <FileVideo size={16} />
                                                            </div>
                                                            <span className="text-sm font-medium text-gray-700">{lesson.title}</span>
                                                            {lesson.durationSeconds ? (
                                                                <span className="text-xs text-gray-400">({Math.floor(lesson.durationSeconds / 60)}m {lesson.durationSeconds % 60}s)</span>
                                                            ) : null}
                                                        </div>
  ...
  <button 
    onClick={() =>
      setUploadingLessonId(
        uploadingLessonId === lesson.lessonId ? null : lesson.lessonId
      )
    }
  ...
  >
    {uploadingLessonId === lesson.lessonId
      ? <><X size={14}/> Đóng</>
      : <>{lesson.videoUrl ? 'Đổi Video' : 'Upload Video'}</>
    }
  </button>
    ...
    <Trash2 size={14} />
                                                    ...
                                                    {uploadingLessonId === lesson.lessonId && (
                                                        <div className="mt-4 pl-10 border-t pt-4">
                                                            <VideoUploader lessonId={lesson.lessonId} onUploadComplete={() => { setUploadingLessonId(null); refetch(); }} />
                                                        </div>
                                                    )}
                                                </div>
                                            )}
                          ...
                          {addingLessonToModuleId === module.moduleId ? (
                            <div className="ml-10 flex gap-2 items-center mt-3 animate-in fade-in">
                              <input
                                ...
                                onKeyDown={(e) => e.key === 'Enter' && handleCreateLesson(module.moduleId, module.lessons)}
                              />
                              <button onClick={() => handleCreateLesson(module.moduleId, module.lessons)} className="bg-blue-600 text-white px-3 py-2 rounded text-sm hover:bg-blue-700">Thêm</button>
                              <button onClick={() => setAddingLessonToModuleId(null)} className="text-gray-500 px-3 py-2 text-sm hover:bg-gray-100">Hủy</button>
                            </div>
                          ) : (
                            <button
                              onClick={() => { setAddingLessonToModuleId(module.moduleId); setNewLessonTitle(''); }}
      ...
      {isAddingModule && (
          <div className="mt-4 border-2 border-dashed border-blue-300 bg-blue-50 p-4 rounded-xl flex gap-3 animate-in fade-in">
            ...
            />
            <button onClick={handleCreateModule} className="bg-blue-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-blue-700">Lưu</button>
            <button onClick={() => setIsAddingModule(false)} className="bg-white border text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-50">Hủy</button>
          </div>
      )}
    </div>
  );
